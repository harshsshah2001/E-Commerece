    ...
    <div class="container">
      ...
      <div class="swiper product-swiper open-up" data-aos="zoom-out">
        <div class="swiper-wrapper d-flex">
          ...
          $selling_sql = "SELECT * FROM `best_selling_items`";
          $selling_data = mysqli_query($conn, $selling_sql);

          if (mysqli_num_rows($selling_data) > 0) {
            while ($row = mysqli_fetch_assoc($selling_data)) {

              echo '<div class="swiper-slide">
              <div class="product-item image-zoom-effect link-effect">
                <div class="image-holder">
                  <a href="index.html">
                    <img src="uploads/' . htmlspecialchars($row['image']) . '" alt="categories" class="product-image img-fluid">
                  </a>
                  <a href="./User/cart.php?name=' . $row['name'] . '&price=' . $row['price'] . '&image=' . $row['image'] . '" class="btn-icon btn-wishlist">
                    <svg width="24" height="24" viewBox="0 0 24 24">
                      <use xlink:href="#heart"></use>
                    </svg>
                  </a>
                  <div class="product-content">
                    <h5 class="text-uppercase fs-5 mt-3">
                        ' . $row['name'] . '             </h5>
                    <a href="index.html" class="text-decoration-none" data-after="Add to cart"><span>' . htmlspecialchars($row['price']) . '</span></a>
                  </div>
                </div>
              </div>
            </div>';
