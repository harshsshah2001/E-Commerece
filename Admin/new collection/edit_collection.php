...
// Fetch the user data
$sql = "SELECT * FROM add_collection WHERE id = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    ...
    if ($image && move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
        $update_query = "UPDATE add_collection  
                         SET title = '$title', paragraph = '$paragraph', image = '$image' 
                         WHERE id = $id";
    } else {
        $update_query = "UPDATE  add_collection
                         SET title = '$title', paragraph = '$paragraph'
                         WHERE id = $id";
    }
...
<body>
    <div class="container mt-5">
        ...
        <form action="edit_collection.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="paragraph" name="paragraph" value="<?php echo htmlspecialchars($user['paragraph']); ?>" required>
            </div>
            ...
            <button type="submit" name="submit" class="btn btn-primary">Update Collection</button>
